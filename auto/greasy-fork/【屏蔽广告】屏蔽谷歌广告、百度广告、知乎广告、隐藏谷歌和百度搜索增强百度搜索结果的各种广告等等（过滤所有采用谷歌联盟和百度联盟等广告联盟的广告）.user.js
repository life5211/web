...
// @require https://www.dadiyouhui02.cn/tampermonkey/coment/baiduga.js?version=795299
// @updateURL  https://www.dadiyouhui02.cn/tampermonkey/baiduga.user.js
// @downloadURL  https://www.dadiyouhui02.cn/tampermonkey/baiduga.user.js
// @grant        GM_addStyle
// @grant        GM_download
...
2023.06.19  强力模式增加对页面悬浮广告的删除能力。能屏蔽大部分牛皮癣广告了。
如果误杀请自行关闭强力模式

2023.05.27 修复youtube的下载
2023.04.22 修复一些BUG和百度搜索页面的显示错误的问题
增加对简书的广告过滤和某MP4下载站的广告过滤
2022.11.21 修复twitter和Youtube的下载
2022.11.10 移动端百度搜索页面增加网页图标
2022.11.3 支持ios和mac的Safari，Safari推荐使用stay脚本管理器，和userscripts有兼容问题。不能用
2022.10.14 修复部分bug， 知乎评论可以按屏蔽关键词屏蔽了
2022.9.3 隐藏樱花动漫广告dmh8.com
2022.8.29 搜索页面美化，默认关闭。需要可自行在控制面板开启
2022.8.23 修复谷歌搜索的关键词和网址屏蔽，显示搜索结果里的网站图标，优化页面
2022.8.13 默认最小化b站直播间那个22娘，免得影响影响看直播。优化部分页面的广告
2022.8.05 新增试验功能强力过滤广告，此功能默认关闭，有需要可在控制面板自行开启。屏蔽网站引用的【外站】iframe以达到屏蔽广告的效果。外站的定义就是A域名网站引用B域名网站的iframe页面。（此项可能会造成部分正常引用的iframe不显示，设置白名单可以解决。同时已把常见网站正常的iframe加入了白名单以防不能正常使用，例如各大视频网站和谷歌腾讯阿里的图形验证等等）。
2022.6.19 修复X浏览器百度页面翻页问题
2022.6.7 进一步优化手机端的体验。修复360搜索的广告过滤
2022.6.5 修复B站搜索结果过滤。允许自定义是否过滤百度首页热搜，百家号热搜。屏蔽bing的广告
2022.5.10 新增去csdn广告，无需关注自动展开全文
2022.5.6 优化了百度知道百度文档部分广告。百度知道可直接显示需要付费才能查看的内容了。修复手机端上的部分bug
2022.3.11 修复部分bug，感谢LARA_SSR的反馈
...
function kgpingbisinfo() {
  ...
  swal({
   title: "欢迎您、插件已针对大部分网站通用的广告样式进行屏蔽美化",
    text: "本插件提供了很多自定义功能，可以自由开启/关闭相应功能，本次更新新增了强力模式。强力模式下会禁用大部分第三方的iframe。并且会过滤大部分网站的图片悬浮广告。建议开启增加广告过滤效果",
    icon: "success",
    buttons: ['暂时不用', '开启强力模式'],
  }).then((value) => {
    if (value) {
   let allconfig = GM_getValue('allconfig') || {};
      ...
      GM_setValue('allconfig', allconfig);
       swal({
      title: "已为您开启强力模式。过滤规则后续会继续更新，过滤规则可能不兼容所有的页面。如果页面显示不正常可将当前页面加入白名单、或者在控制面板-高级功能-强力模式关闭",
      icon: "info",
    });
    } else {
    swal({
      title: "您已取消开启强力模式,如果想体验强力模式可在控制面板-高级功能-强力模式开启",
      icon: "info",
    });
  }
  });
...
GM_addStyle(tab_bing_tabcss);
      ...
      function deltopiframe() {
    	if ($("iframe[src*='//googleads']").length>0 || $("iframe[src*='pos.baidu.com']").length>0){
    		goingnumb=3;
  	       	   }
...
$("div[class='article_content']").css({"overflow":"auto" ,"height":"auto"});
 ...
 if(zkdurldomain.indexOf(weibourl) >= 0  ){
 	 	  ...
 	 	  $("div[class*='woo-box-flex woo-box-alignCenter woo-box-justifyCenter TipsAd").remove();
...
function delgoogle() {
	  ...
	  if(zkdurldomain.indexOf('google.com') > 0  && zkdurldomain.indexOf('/search?') > 0){
    	   ...
    	   if ( $('#bingconfig_btn').length>0){
  	   	    ...
  	   	    $('#hdtb-tls').before('<a id="bingconfig_btn" class="gb_Sc" style="border: 1px solid transparent;   text-align: center;    border-radius: 2px;    line-height: 19px;    cursor: pointer;    margin-left: -1px;    padding: 4px 11px;  margin-right: -11px;"  href="'+configm+'"  >过滤设置</a>');
  	   }
...
function delbaidu() {
 ...
 if ( $("span[class='s-menu-item s-menu-mine current']").length > 0  ){
 	 $("span[class='s-menu-item s-menu-mine current']").attr('current',1);
